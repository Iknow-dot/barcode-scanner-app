...
def main() -> int:
    ...
    lines += [
        ...
        "**Caveat: `pg_stat_statements` is cumulative for the whole stack, not",
        "scoped to this run.** It is never reset automatically, so on a stack",
        "that hasn't had `pg_stat_statements_reset()` run since it last started,",
        "this table is dominated by migrations, `collectstatic`, and",
        "`seed_loadtest`'s own `bulk_create`s rather than by the k6 run that",
        "produced the CSV above. Run `SELECT pg_stat_statements_reset();` right",
        "after seeding and immediately before the k6 invocation you want this",
        "table to reflect (see loadtest/README.md's \"Reading a run\" section) if",
        "these numbers need to mean just that one run.", "",
