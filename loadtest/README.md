...
- Everything `k6/lib/config.js` reads from `__ENV` can be overridden by
  exporting the variable (or prefixing the invocation) before calling the
  wrapper, e.g. `PRODUCT_COUNT=500 bash loadtest/scripts/k6.sh entry/smoke.js`.
...
| `SWEEP_RATE` / `SWEEP_DURATION` / `GRID_SIZE` | `5` / `40s` / `20` | `entry/sweep.js` per-scenario rate, window length, and the `images` scenario's grid width. |
