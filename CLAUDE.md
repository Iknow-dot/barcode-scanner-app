...
- **Error response shape** — backend errors that the frontend needs to translate or branch on use `{"code": "MACHINE_READABLE_CODE", "detail": "human text", ...}`. Follow this when adding new error responses (see `EXTERNAL_SERVICE_*`, `IP_NOT_ALLOWED`, `USER_LIMIT_REACHED`, `RS_GE_*`, `NO_ORGANIZATION` for examples). `User.save()` calls `full_clean()` and DRF does **not** translate a Django `ValidationError`, so a write that can trip `User.clean()` (an org or role change) must go through `users.serializers.save_user` or it is a 500, not a 400.
